...
type CashuMintSpec struct {
	...
	// wrapper images (e.g. an attested-entrypoint) can be configured without
	// adding provider-specific fields.
	// +optional
	ExtraEnv []corev1.EnvVar `json:"extraEnv,omitempty"`

	// ExtraVolumes appends additional pod volumes to the mint Deployment.
	// Use this for provider-specific read-only devices or metadata files that
	// are not modeled as first-class operator fields.
	// +optional
	ExtraVolumes []corev1.Volume `json:"extraVolumes,omitempty"`

	// ExtraVolumeMounts appends additional volume mounts to the mintd container.
	// The referenced volumes must be present in ExtraVolumes or generated by the operator.
	// +optional
